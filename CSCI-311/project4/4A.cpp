...
bool canPartitionHelper( vector<int>& nums, int n, int target) {
    ...
    return canPartitionHelper(nums, n - 1, target - nums[n - 1]) ||
           canPartitionHelper(nums, n - 1, target);
}
...
bool canPartition( vector<int>& nums) {
    ...
    return canPartitionHelper(nums, nums.size(), totalSum / 2);
}
